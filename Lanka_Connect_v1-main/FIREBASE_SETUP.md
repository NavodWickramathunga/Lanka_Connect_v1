...
## 4) Deploy Firestore + Storage + Functions
...
```bash
powershell -ExecutionPolicy Bypass -File scripts/firebase_deploy_backend.ps1
```
...
Do not commit sensitive Firebase platform config files (`android/app/google-services.json`, `ios/Runner/GoogleService-Info.plist`).
